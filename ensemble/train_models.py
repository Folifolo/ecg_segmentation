...
if __name__ == "__main__":
    ...
    load = False  #загрузка по name, если обучение было прервано

    xy = load_dataset()

    if load == True:
        pkl_filename = path_to_data + "\\trim_" + str(name) + ".pkl"
    ...
    while len(xtrain)> 0:
        ...
        xtrain, ytrain = trim(tmp_model, xtrain, ytrain, str(name), threshold, path_to_data, win_len)
